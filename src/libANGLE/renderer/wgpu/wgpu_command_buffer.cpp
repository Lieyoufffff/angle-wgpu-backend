...
void CommandBuffer::setBindGroup(uint32_t groupIndex,
                                 ...
                                 const uint32_t *dynamicOffsets)
{
    ...
    setBindGroupCommand->dynamicOffsetCount =
        std::min(dynamicOffsetCount, static_cast<uint32_t>(3u));
    for (uint32_t i = 0; i < setBindGroupCommand->dynamicOffsetCount; ++i)
    {
        setBindGroupCommand->dynamicOffsets[i] = dynamicOffsets[i];
    }
    for (uint32_t i = setBindGroupCommand->dynamicOffsetCount; i < 3; ++i)
    {
        setBindGroupCommand->dynamicOffsets[i] = 0;
    }
}
            ...
            switch (CurrentCommandID(currentCommand))
            {
                ...
                case CommandID::SetBindGroup:
                {
                    ...
                    wgpu->renderPassEncoderSetBindGroup(
                        encoder.get(), setBindGroupCommand.groupIndex,
                        setBindGroupCommand.bindGroup,
                        setBindGroupCommand.dynamicOffsetCount,
                        setBindGroupCommand.dynamicOffsetCount > 0
                            ? setBindGroupCommand.dynamicOffsets
                            : nullptr);
                    break;
                }
