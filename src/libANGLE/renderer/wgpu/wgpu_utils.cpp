...
WGPUCullMode GetCullMode(gl::CullFaceMode mode, bool cullFaceEnabled)
{
    ...
    switch (mode)
    {
        ...
        case gl::CullFaceMode::FrontAndBack:
            // WebGPU has no FrontAndBack cull mode. Returning None is incorrect
            // (should discard all triangles), but this mode is extremely rare in practice.
            // Proper fix: early-return in setupDraw when FrontAndBack is active.
            return WGPUCullMode_None;
        default:
            UNREACHABLE();
            return WGPUCullMode_None;
...
WGPUBlendFactor GetBlendFactor(gl::BlendFactorType blendFactor)
{
    switch (blendFactor)
    {
        ...
        case gl::BlendFactorType::ConstantAlpha:
            // WebGPU has no ConstantAlpha factor. Using Constant as approximation:
            // correct only when blend color RGB == A. Full fix requires rewriting
            // blend color to (a,a,a,a) when this factor is active.
            return WGPUBlendFactor_Constant;

        case gl::BlendFactorType::OneMinusConstantAlpha:
            // Same limitation as ConstantAlpha above.
            return WGPUBlendFactor_OneMinusConstant;
...
WGPUTextureViewDimension GetWgpuTextureViewDimension(gl::TextureType textureType)
{
    switch (textureType)
    {
        ...
        default:
            // External/Buffer/Rectangle textures - use 2D view as fallback
            return WGPUTextureViewDimension_2D;
    }
}
...
WGPUAddressMode GetSamplerAddressMode(const GLenum wrap)
{
    switch (wrap)
    {
        ...
        case GL_CLAMP_TO_BORDER:
            // Not in WebGPU; fall back to ClampToEdge (acceptable for ES 3.0)
...
uint32_t GetFirstIndexForDrawCall(gl::DrawElementsType indexType, const void *indices)
{
    ...
    if (indexBufferByteOffset % indexSize != 0)
    {
        ...
        // Return aligned offset; unaligned access handled via index buffer copy if needed.
    }
