...
{
  public:
    webgpu::SamplerHandle getSampler(const DawnProcTable *wgpu,
                                     webgpu::DeviceHandle device,
                                     const WGPUSamplerDescriptor &desc);
    void clear() { mCache.clear(); }

  private:
    static size_t hashDescriptor(const WGPUSamplerDescriptor &desc);
    std::unordered_map<size_t, webgpu::SamplerHandle> mCache;
};
...
class ContextWgpu : public ContextImpl
{
  ...
  private:
    ...
    // TODO(anglebug.com/376553328): ring buffer for batching uniform uploads.
    // Once fully integrated, updateUniformsAndGetBindGroup() will sub-allocate
    // from this buffer instead of creating a new WGPUBuffer per draw.
    // webgpu::UniformRingBuffer mUniformRingBuffer;  // Uncomment when fully integrated
