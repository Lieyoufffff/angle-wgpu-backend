        ...
        *adjustedIndicesPtr = 0;
        ...
        switch (indexDataNeedsStreaming)
        {
            ...
            case IndexDataNeedsStreaming::TriangleFanStreaming:
                ANGLE_TRY(streamIndicesTriangleFan(
                    contextWgpu, sourceDrawElementsTypeOrInvalid, destDrawElementsTypeOrInvalid,
                    count, indices, stagingData, destIndexBufferSize, indexRange, srcIndexData,
                    &stagingBuffer, &stagingUploads, &currentStagingDataPosition));
                break;
...
// Triangle fan emulation: convert fan indices to triangle list.
// For a fan with vertices [v0, v1, v2, ..., vN], generate triangles:
// (v0, v1, v2), (v0, v2, v3), ..., (v0, v(N-1), vN)
angle::Result VertexArrayWgpu::streamIndicesTriangleFan(
    ContextWgpu *contextWgpu,
    gl::DrawElementsType sourceDrawElementsTypeOrInvalid,
    gl::DrawElementsType destDrawElementsTypeOrInvalid,
    GLsizei count,
    ...
    size_t *currentStagingDataPositionOut)
{
    ...
    size_t outIndex = *currentStagingDataPositionOut;

    if (sourceDrawElementsTypeOrInvalid == gl::DrawElementsType::InvalidEnum)
    {
        // DrawArrays case: generate sequential triangle fan indices.
        uint32_t startVertex = static_cast<uint32_t>(indexRange->start());
        for (GLsizei i = 2; i < count; i++)
        {
            uint32_t idx0 = startVertex;
            uint32_t idx1 = startVertex + static_cast<uint32_t>(i - 1);
            uint32_t idx2 = startVertex + static_cast<uint32_t>(i);
            memcpy(stagingData + outIndex, &idx0, destIndexUnitSize);
            outIndex += destIndexUnitSize;
            memcpy(stagingData + outIndex, &idx1, destIndexUnitSize);
            outIndex += destIndexUnitSize;
            memcpy(stagingData + outIndex, &idx2, destIndexUnitSize);
            outIndex += destIndexUnitSize;
        }
    }
    else
    {
        // DrawElements case: read source indices and fan-expand them.
        auto readIndex = [&](GLsizei i) -> uint32_t {
            switch (sourceDrawElementsTypeOrInvalid)
            {
                case gl::DrawElementsType::UnsignedByte:
                    return static_cast<uint32_t>(srcIndexData[i]);
                case gl::DrawElementsType::UnsignedShort:
                    return static_cast<uint32_t>(
                        reinterpret_cast<const uint16_t *>(srcIndexData)[i]);
                case gl::DrawElementsType::UnsignedInt:
                    return reinterpret_cast<const uint32_t *>(srcIndexData)[i];
                default:
                    UNREACHABLE();
                    return 0;
            }
        ...
        uint32_t fanCenter = readIndex(0);
        for (GLsizei i = 2; i < count; i++)
        {
            uint32_t idx1 = readIndex(i - 1);
            uint32_t idx2 = readIndex(i);
            memcpy(stagingData + outIndex, &fanCenter, destIndexUnitSize);
            outIndex += destIndexUnitSize;
            memcpy(stagingData + outIndex, &idx1, destIndexUnitSize);
            outIndex += destIndexUnitSize;
            memcpy(stagingData + outIndex, &idx2, destIndexUnitSize);
    ...
    stagingUploadsOut->push_back({*currentStagingDataPositionOut, stagingBufferPtr,
                                  &mStreamingIndexBuffer, 0, destIndexBufferSize});
    *currentStagingDataPositionOut = outIndex;
