    ...
    webgpu::BindGroupHandle *outBindGroup)
{
    if (mDefaultUniformBlocksDirty.any())
    {
        ...
        //
        // Optimization plan (Uniform Batching / Ring Buffer):
        //   1. Replace per-draw buffer creation with sub-allocation from
        //      ContextWgpu::mUniformRingBuffer.allocate(requiredSpace, &dynamicOffset)
        //   2. memcpy uniform data to ringBuffer.getMappedPointer() + dynamicOffset
        //   3. Create bind group with hasDynamicOffset=true (only rebuild when ring buffer resets)
        //   4. Pass dynamicOffset via SetBindGroupCommand's dynamicOffsets field
        //   5. Call ringBuffer.reset() at frame boundary / command submit
        // This reduces per-draw allocation from O(N) buffer creates to O(1) pointer bump.
    ...
    if (mSamplerBindingsDirty)
    {
             ...
             ++textureIndex)
        {
                 ...
                 ++arrayElement)
            {
                ...
                if (!texture)
                {
                    // Incomplete texture: GL spec says results are undefined.
                    // Skip this draw rather than crash WebGPU validation.
                    return angle::Result::Continue;
                }
                const gl::SamplerState *samplerState =
                    sampler ? &sampler->getSamplerState() : &texture->getSamplerState();
                // Shadow samplers use comparison sampler type (already set in layout)
                // and WGPUTextureSampleType_Depth (already returned by GetTextureSampleType).
                // The bind group creation below handles them correctly.
                TextureWgpu *textureWgpu = webgpu::GetImpl(texture);

                WGPUSamplerDescriptor sampleDesc  = gl_wgpu::GetWgpuSamplerDesc(samplerState);
                webgpu::SamplerHandle wgpuSampler =
                    contextWgpu->getSamplerCache().getSampler(wgpu, contextWgpu->getDevice(), sampleDesc);
        ...
        // Compute content hash from bound samplers and texture views
        size_t bindingsHash = 0;
        auto combineHash = [&bindingsHash](size_t val) {
            bindingsHash ^= val + 0x9e3779b9 + (bindingsHash << 6) + (bindingsHash >> 2);
        };
        for (const auto &entry : bindings)
        {
            combineHash(std::hash<const void *>()(entry.sampler));
            combineHash(std::hash<const void *>()(entry.textureView));
        }

        if (bindingsHash != mLastSamplerBindGroupHash || !mSamplersAndTexturesBindGroup)
        {
            WGPUBindGroupDescriptor bindGroupDesc = WGPU_BIND_GROUP_DESCRIPTOR_INIT;
            ...
            mSamplersAndTexturesBindGroup = webgpu::BindGroupHandle::Acquire(
                wgpu, wgpu->deviceCreateBindGroup(contextWgpu->getDevice().get(), &bindGroupDesc));
            mLastSamplerBindGroupHash = bindingsHash;
        }
