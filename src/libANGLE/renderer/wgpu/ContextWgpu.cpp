...
size_t SamplerCache::hashDescriptor(const WGPUSamplerDescriptor &desc)
{
    size_t hash = 0;
    auto combine = [&hash](size_t val) { hash ^= val + 0x9e3779b9 + (hash << 6) + (hash >> 2); };
    combine(static_cast<size_t>(desc.addressModeU));
    combine(static_cast<size_t>(desc.addressModeV));
    combine(static_cast<size_t>(desc.addressModeW));
    combine(static_cast<size_t>(desc.magFilter));
    combine(static_cast<size_t>(desc.minFilter));
    combine(static_cast<size_t>(desc.mipmapFilter));
    combine(std::hash<float>()(desc.lodMinClamp));
    combine(std::hash<float>()(desc.lodMaxClamp));
    combine(static_cast<size_t>(desc.compare));
    combine(static_cast<size_t>(desc.maxAnisotropy));
    return hash;
}

webgpu::SamplerHandle SamplerCache::getSampler(const DawnProcTable *wgpu,
                                               ...
                                               const WGPUSamplerDescriptor &desc)
{
    ...
    webgpu::SamplerHandle sampler =
        webgpu::SamplerHandle::Acquire(wgpu, wgpu->deviceCreateSampler(device.get(), &desc));
    mCache[key] = sampler;
    return sampler;
}
                                      ...
                                      GLsizei count)
{
    if (mode == gl::PrimitiveMode::TriangleFan)
    {
        ...
        ANGLE_TRY(setupDraw(context, mode, first, count, 1,
                            gl::DrawElementsType::InvalidEnum, nullptr, 0,
    ...
    if (mode == gl::PrimitiveMode::LineLoop || mode == gl::PrimitiveMode::TriangleFan)
    {
        mCommandBuffer.drawIndexed(indexCount, 1, firstIndex, 0, 0);
    }
    else
...
angle::Result ContextWgpu::drawArraysIndirect(const gl::Context *context,
                                              ...
                                              const void *indirect)
{
    // CPU-side emulation: read the indirect command and dispatch a regular instanced draw.
    const gl::DrawArraysIndirectCommand *cmd =
        static_cast<const gl::DrawArraysIndirectCommand *>(indirect);
    return drawArraysInstancedBaseInstance(context, mode, cmd->first, cmd->count,
...
angle::Result ContextWgpu::drawElementsIndirect(const gl::Context *context,
                                                ...
                                                const void *indirect)
{
    // CPU-side emulation: read the indirect command and dispatch a regular indexed draw.
    const gl::DrawElementsIndirectCommand *cmd =
        static_cast<const gl::DrawElementsIndirectCommand *>(indirect);
    GLsizei count        = static_cast<GLsizei>(cmd->count);
    GLsizei instanceCount = static_cast<GLsizei>(cmd->primCount);
    // Compute the byte offset into the index buffer from firstIndex.
    const void *indices =
        reinterpret_cast<const void *>(static_cast<uintptr_t>(cmd->firstIndex) *
                                       gl::GetDrawElementsTypeSize(type));
    return drawElementsInstancedBaseVertexBaseInstance(context, mode, count, type, indices,
...
angle::Result ContextWgpu::multiDrawArraysInstanced(const gl::Context *context,
                                                    ...
                                                    GLsizei drawcount)
{
    return rx::MultiDrawArraysInstancedGeneral(this, context, mode, firsts, counts,
...
angle::Result ContextWgpu::handleDirtyBlendConstant(DirtyBits::Iterator *dirtyBitsIterator)
{
    ...
    // WebGPU has no ConstantAlpha blend factor (same limitation as D3D11).
    // When GL uses GL_CONSTANT_ALPHA, we map to WGPUBlendFactor_Constant and rewrite the
    // blend color to (a,a,a,a) so Constant reads alpha on all channels.
    // Reference: D3D11 StateManager11.cpp uses identical pattern with emulateConstantAlpha.
    bool emulateConstantAlpha = mState.hasConstantAlphaBlendFunc();
    float r = emulateConstantAlpha ? blendColor.alpha : blendColor.red;
    float g = emulateConstantAlpha ? blendColor.alpha : blendColor.green;
    float b = emulateConstantAlpha ? blendColor.alpha : blendColor.blue;
    float a = blendColor.alpha;

    bool isDefaultBlendConstant = r == 0 && g == 0 && b == 0 && a == 0;
    ...
    ASSERT(mCurrentGraphicsPipeline);
    mCommandBuffer.setBlendConstant(r, g, b, a);
    return angle::Result::Continue;
}
