...
{
    // Use dawn::native synchronous API for device creation.
    // The webgpu.h async callback path (instanceWaitAny) can hang with SwiftShader
    // in headless environments due to slow CPU-based Vulkan device initialization.
    webgpu::CreateDeviceSyncResult result = webgpu::CreateDeviceSync();
    if (!result.success)
    {
        return egl::Error(EGL_NOT_INITIALIZED, "Failed to create WebGPU device via dawn::native");
    }

    mProcTable = webgpu::GetDefaultProcTable();
    mDevice    = webgpu::DeviceHandle::Acquire(&mProcTable, result.device);
    mAdapter   = webgpu::AdapterHandle::Acquire(&mProcTable, result.adapter);
    mInstance  = webgpu::InstanceHandle::Acquire(&mProcTable, result.instance);
