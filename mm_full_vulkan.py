...
# NO ANGLE_FORCE_WEBGPU - use default Vulkan backend
...
# Verify backend (should be Vulkan)
...
# Select ALL MotionMark suite tests
...
return {btnDisabled: btn.disabled, selected: suitesManager.isAtLeastOneTestSelected()};
""")
...
# Start
print("Starting full MotionMark suite (Vulkan backend)...", flush=True)
...
print("\n" + "="*60, flush=True)
print(" MotionMark 1.3.1 - Vulkan Backend (Full Suite)", flush=True)
print("="*60, flush=True)
...
with open("/root/motionmark_bench/vulkan_full_results.json", "w") as f:
    json.dump(data, f, indent=2, ensure_ascii=False)
print("\nSaved to vulkan_full_results.json", flush=True)

driver.save_screenshot("/root/motionmark_bench/vulkan_full_screenshot.png")
driver.quit()
